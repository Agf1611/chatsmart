<x-layout-dashboard title="Campaigns">
    ...
    <section class="section-hero-card mb-4">
        <div class="d-flex flex-column flex-lg-row justify-content-between gap-4 align-items-lg-center">
            <div>
                <p class="section-kicker mb-2">Campaign Center</p>
                <h3 class="section-title mb-2">Riwayat Campaign & eksekusi blast</h3>
                <p class="hero-meta mb-0">Pantau status campaign, cek ringkasan pengiriman, dan lanjutkan workflow
                    tanpa keluar dari dashboard utama.</p>
            </div>
            <div class="toolbar-actions">
                <a href="{{ route('campaign.create') }}" class="btn btn-primary">
                    <i class="bi bi-plus-lg me-2"></i>Campaign Baru
                </a>
                <button onclick="clearCampaign()" type="button" class="btn btn-outline-danger">
                    <i class="bi bi-trash-fill me-2"></i>Clear Campaign
                </button>
            </div>
        </div>
    </section>
    ...
    <div class="row g-4">
        <div class="col-12 col-lg-8 d-flex">
            <div class="smart-list-card w-100">
                <div class="card-header py-3 px-4">
                    <h5 class="mb-0">Daftar Campaign</h5>
                </div>
                        ...
                        <table class="table align-middle">
                            ...
                            <tbody>
                                @if ($campaigns->total() == 0)
                                    <tr>
                                        <td colspan="7">
                                            <div class="smart-empty">Belum ada campaign yang tersedia.</div>
                                        </td>
                                    </tr>
                                @endif
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a onclick="viewMessage('{{ $campaign->id }}')" href="#"
                                                class="text-info" data-bs-toggle="tooltip" title="Views Message">
                                                <i class="bi bi-eye-fill"></i> {{ $campaign->type }}
                                            </a>
                                        </td>
                                        <td>{{ $campaign->schedule }}</td>
                                        <td>
                                            <div class="d-flex flex-column gap-1">
                                                <span>{{ $campaign->blasts_count }} <span class="badge bg-primary">total</span></span>
                                                <span>{{ $campaign->blasts_success }} <span class="badge bg-success">Success</span></span>
                                                <span>{{ $campaign->blasts_failed }} <span class="badge bg-danger">Failed</span></span>
                                                <span>{{ $campaign->blasts_pending }} <span class="badge bg-warning text-dark">Waiting</span></span>
                                            </div>
                                        </td>
                                        <td>
                                            ...
                                            @else
                                                <span class="badge rounded-pill bg-danger">{{ $campaign->status }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="d-flex align-items-center gap-3 fs-6">
                                                <a href="{{ route('campaign.blasts', $campaign->id) }}" class="text-primary"
                                                    data-bs-toggle="tooltip" title="View Data">
                                                    <i class="bi bi-eye-fill"></i>
                                                </a>
                                                @if ($campaign->status == 'processing' || $campaign->status == 'waiting')
                                                    <a href="#" onclick="pauseCampaign('{{ $campaign->id }}')"
                                                        class="text-warning" data-bs-toggle="tooltip" title="Pause">
                                                        <i class="bi bi-pause-fill"></i>
                                                    </a>
                                                @endif
                                                @if ($campaign->status == 'paused')
                                                    <a href="#" onclick="resumeCampaign('{{ $campaign->id }}')"
                                                        class="text-success" data-bs-toggle="tooltip" title="Resume">
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <li class="page-item {{ $campaigns->currentPage() == 1 ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $campaigns->previousPageUrl() }}">{{ __('system.previous') }}</a>
                            </li>
                            ...
                            <li class="page-item {{ $campaigns->currentPage() == $campaigns->lastPage() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $campaigns->nextPageUrl() }}">{{ __('system.next') }}</a>
                            </li>
                        </ul>
                    </nav>
                </div>
        ...
        <div class="col-12 col-lg-4 d-flex">
            <div class="smart-filter-card w-100 p-4">
                <h5 class="mb-3">Filter Campaign</h5>
                <form class="row g-3">
                    <div class="col-12">
                        <label class="form-label">Device</label>
                        <input value="{{ request()->has('device') ? request()->device : '' }}" type="number"
                            name="device" class="form-control" placeholder="Nomor device">
                    </div>
                    <div class="col-12">
...
<script>
    ...
    function pauseCampaign(id) {
        $.ajax({
            ...
            error: () => toastr['error']('something went wrong when pausing campaign')
        })
    }
    ...
    function resumeCampaign(id) {
        $.ajax({
            ...
            dataType: 'json',
            success: () => location.reload(),
            error: () => toastr['error']('something went wrong when resuming campaign')
        })
    }
    ...
    function deleteCampaign(id) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: () => location.reload(),
            error: () => toastr['error']('something went wrong when deleting campaign ')
        })
    }
    ...
    function clearCampaign() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: () => location.reload(),
            error: () => toastr['error']('something went wrong when clearing campaign ')
        })
    }
</script>
