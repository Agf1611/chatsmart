...
class SendMessageRequest extends FormRequest
{
    ...
    {
        $sender = preg_replace('/\s+/', '', (string) $this->input('sender', ''));
        ...
        $rules = [
            'sender' => [
                ...
                function ($attribute, $value, $fail) {
                    $device = $this->user()?->devices()->where('body', $value)->first();
                    if (!$device) {
                        $fail(
                ...
                function ($attribute, $value, $fail) {
                    ...
                    foreach ($value as  $val) {
                        ...
                        if ($type != 'call' && $type != 'url') {
                            $fail('Invalid type for ' . $attribute . ' ' . $i . ' must be call or url like this : call|+6281234567891|Call me or url|https://google.com|Visit Google');
                            return;
                        }
                    }
                }
