...
class WhatsappServiceImpl implements WhatsappService
{
    ...
    private function normalizeMediaUrl(?string $mediaUrl): ?string
    {
        ...
        if (preg_match('/^[A-Za-z]:\\\\/', $mediaUrl) || str_starts_with($mediaUrl, '\\\\')) {
            return $mediaUrl;
        }
    ...
    {
        $mediaUrl = $this->normalizeMediaUrl($request->url);
        // GET FILE NAME from $request->url
        $fileName = explode('/', (string) $mediaUrl);
    ...
    public function sendButton($request, $receiver): object | bool
    {
        ...
        $image = $this->normalizeMediaUrl($image);
        $data = [
            'token' => $request->sender,
            'number' => $receiver,
